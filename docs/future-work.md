...
| **M4** | File transfer | Sshx supports `scp`-style file upload. Iroh already has a blob API (`iroh-blobs`) that fits naturally. | New `Upload { name, mime, size }` message → agent returns a blob ticket → client streams bytes on a new substream. ~400 LoC + UI: drag-drop on the terminal pane. |
